...
僅依照提供的資料，創建一個口述影像腳本JSON文件，儘量貼近原作品再現的原則，並盡量避開角色對話的時間，著重於劇情相關的畫面描述。無須描述對話，其中包含一個audioDescriptions列表，每個audioDescriptions對象包含一個時間戳(hh:mm:ss.ms)和繁體中文口述影像。時間戳應該是一個日期時間對象，內容應該是一個字符串。例如：
{{schema}}
